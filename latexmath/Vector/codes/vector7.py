...
#r = 2
#arc_angles = linspace(pi, pi/6, 20)
#arc_xs = r * cos(arc_angles)
#arc_ys = r * sin(arc_angles)
#plt.plot(arc_xs, arc_ys, color = 'red', lw = 1
#plt.gca().annotate('', xy=(10,10), xycoords='data', fontsize=10, rotation =40)
#angle_X = math.degrees(math.acos((XY**2 + XZ**2 - YZ**2) / (2 * XY * XZ)))

#radius = 0.4  # Set the radius of the arc
#center = X    # Set the center of the arc
#angle_start = 0
#angle_end = angle_X
#arc = plt.Circle(center, radius, angle=angle_end-angle_start, fill=False, edgecolor='red')
#plt.gcf().gca().add_artist(arc)
#angle_plot = get_angle_plot(XY, YZ, 2, color="red")
...
# Show the plot
plt.savefig('/sdcard/arduino/Vector/figs/graph2.png')
